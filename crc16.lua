...
function crc_modbus(ucData)
local usp_crc = 0xFFFF
local ucDataSub
  
  for i = 1, #ucData do
    
ucDataSub = ucData[i]
    usp_crc = bit32.bxor(usp_crc, ucDataSub)
  
    for j = 0, 7, 1 do
      if bit32.band(usp_crc, 1) > 0 then
        usp_crc = bit32.rshift(usp_crc, 1)
        usp_crc = bit32.bxor(usp_crc, 0xA001)
      else
        usp_crc = bit32.rshift(usp_crc, 1)
      end
    end
